...
u <- paste0("https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/ilc_mdes01",
            "?format=SDMX-CSV&hhcomp=TOTAL&",
            paste(paste0("geo=", geos), collapse = "&"))
...
if (!ok) {   # fallback: geo-only filter (verified working in Stage 1 probe)
  message("hhcomp filter rejected; falling back to geo-only filter")
  u <- paste0("https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/ilc_mdes01",
              "?format=SDMX-CSV&", paste(paste0("geo=", geos), collapse = "&"))
...
stopifnot(!"BA" %in% d$geo)               # Bosnia absent -> measurement-gap note
